...
| API Key      | API keys are unique to your organization. An API key is required by the Datadog Agent to submit metrics and events to Datadog. Click on `Read more` against `API Key` field in the Datadog Integration Form to get the API Key.|
...
<Thumbnail
  src="/img/graphql/cloud/metrics/datadog-region-select.png"
  alt="Datadog Region Select"
  width="500px"
/>
<Thumbnail
